...
// x-time operation
__device__ inline uint8_t rj_xtime(uint8_t x){
  return (x & 0x80) ? ((x << 1) ^ 0x1b) : (x << 1);
}


// subbyte operation
__device__ inline void aes_subBytes(uint8_t *buf){
...
__device__ inline void aes_subBytes_inv(uint8_t *buf){
  register uint8_t i, b;
  for (i = 0; i < 16; ++i){
    b = buf[i];
    buf[i] = sboxinv[b];
...
__device__ inline void aes_addRoundKey(uint8_t *buf, uint8_t *key){
...
__device__ inline void aes_addRoundKey_cpy(uint8_t *buf, uint8_t *key, uint8_t *cpk){
  register uint8_t i = 16;
  while (i--){
    buf[i] ^= (cpk[i] = key[i]);
    cpk[16+i] = key[16 + i];
...
// shift row operation
__device__ inline void aes_shiftRows(uint8_t *buf){
...
__device__ inline void aes_shiftRows_inv(uint8_t *buf){
  register uint8_t i, j; 
  i = buf[1];
  buf[1] = buf[13];
  buf[13] = buf[9];
...
__device__ inline void aes_mixColumns(uint8_t *buf){
...
__device__ inline void aes_mixColumns_inv(uint8_t *buf){
...
__device__ __host__ inline void aes_expandEncKey(uint8_t *k, uint8_t *rc, const uint8_t *sb){
  register uint8_t i;
...
__device__ inline void aes_expandDecKey(uint8_t *k, uint8_t *rc){
  uint8_t i;
...
int main(){
  ...
  // buf = (uint8_t*)calloc(1048576, sizeof(uint8_t));
