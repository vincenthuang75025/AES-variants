    ...
    for i = 1:11
        if i > 1
            ...
            if i != 11
                for j = 1:4
                    tmp[1] =
                        aes_mul(current[4j-3], 0x2) ⊻ aes_mul(current[4j-2], 0x3) ⊻
                        aes_mul(current[4j-1], 0x1) ⊻ aes_mul(current[4j], 0x1)
                    tmp[2] =
                        aes_mul(current[4j-3], 0x1) ⊻ aes_mul(current[4j-2], 0x2) ⊻
                        aes_mul(current[4j-1], 0x3) ⊻ aes_mul(current[4j], 0x1)
                    tmp[3] =
                        aes_mul(current[4j-3], 0x1) ⊻ aes_mul(current[4j-2], 0x1) ⊻
                        aes_mul(current[4j-1], 0x2) ⊻ aes_mul(current[4j], 0x3)
                    tmp[4] =
                        aes_mul(current[4j-3], 0x3) ⊻ aes_mul(current[4j-2], 0x1) ⊻
                        aes_mul(current[4j-1], 0x1) ⊻ aes_mul(current[4j], 0x2)
                    current[4j-3] = tmp[1]
                    current[4j-2] = tmp[2]
                    current[4j-1] = tmp[3]
                    current[4j] = tmp[4]
